...
// Camera parameters.
shared float4x4 g_mCameraView;		// View matrix
shared float4x4 g_mCameraProj;		// Projection matrix
...
// How fast the particles fade in and out 6.7 is the default
                               ...
                               float age, float normalizedAge)
{
    ...
    return mul(mul(float4(position, 1), g_mCameraView), g_mCameraProj);
}
...
float ComputeParticleSize(float randomValue, float normalizedAge)
{
    ...
    return size * g_mCameraProj._m11;
}
                            ...
                            float randomValue, float normalizedAge)
{
    ...
    // Fade the alpha based on the age of the particle. This curve is used
    ...
    // this looks like. A 6.7 fade scaling factor normalizes the curve so the alpha
    // will just reach all the way up to fully solid.  The fade factor can be adjusted.
    // Anything greater than 6.7 will get to full brightness quicker but will jump to 
    // full brightness and stay there for a while.
